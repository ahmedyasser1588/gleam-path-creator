...
// --- 1. دالة الربط (النسخة النهائية المعتمدة) ---
const getAhmedResponse = async (userText: string) => {
  const apiKey = "AIzaSyCLxQlqheZhHtAxA7ccp3-ciw7DW4k0Ows"; 
  // الرابط ده لازم يكون v1beta عشان يشتغل مع gemini-1.5-flash
  const url = `https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=${apiKey}`;

  const body = {
    contents: [{
      parts: [{
        text: `أنت "أحمد"، مبرمج وحبيب "إسراء". تحدث بالعامية المصرية الدافئة. ردي بحب وحنان على رسالتها: "${userText}"`
      }]
    }]
  };
            ...
            <div className="flex-1 overflow-y-auto px-3 py-3 space-y-3 bg-[#fff5f7]" dir="rtl">
              {messages.map((m, i) => (
                <div key={i} className={`flex ${m.from === "user" ? "justify-start" : "justify-end"}`}>
                  <div className={`max-w-[85%] rounded-2xl px-3 py-2 text-sm ${
                      m.from === "user" ? "bg-white text-gray-700 border" : "bg-pink-500 text-white"
                    }`}>
                    {m.text}
                  </div>
                </div>
              ...
              {typing && (
                <div className="flex justify-end">
                  <div className="bg-pink-100 text-pink-600 rounded-2xl px-3 py-2 text-xs animate-pulse">أحمد بيكتب...</div>
                </div>
              )}
              <div ref={endRef} />
            </div>
