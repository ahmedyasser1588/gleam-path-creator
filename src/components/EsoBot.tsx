...
const getAhmedResponse = async (userText: string) => {
  ...
  // الرابط ده هو الأدق حالياً لموديل 1.5 flash
    ...
    contents: [
      {
        parts: [
          {
            text: `أنت "أحمد". مبرمج مصري وحبيب "إسراء". تحدث بالعامية المصرية الدافئة فقط. كن حنوناً جداً ومرحاً وداعماً. ردي بحب على رسالة إسراء: "${userText}"`
          }
        ]
      }
    ]
  ...
  try {
    ...
    return "فيه مشكلة في الربط يا قلبي.. قولي تاني؟";
  }
};
      ...
      <AnimatePresence>
        {!isOpen && (
          <motion.button
            ...
            className="fixed bottom-6 left-6 z-50 w-14 h-14 rounded-full bg-pink-500 text-white flex items-center justify-center shadow-lg hover:bg-pink-600 transition-colors"
            initial={{ scale: 0 }}
            animate={{ scale: 1 }}
          >
          ...
          >
            ...
            <div className="flex items-center gap-2 px-3 py-3 border-t border-pink-50 bg-white">
              ...
              <button 
                onClick={handleSend}
                disabled={typing}
                className="w-10 h-10 rounded-full bg-pink-500 text-white flex items-center justify-center hover:bg-pink-600 transition-colors disabled:opacity-50"
              >
