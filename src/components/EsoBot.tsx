...
const getAhmedResponse = async (userText: string) => {
  // حط المفتاح الجديد هنا يا أحمد
  const apiKey = "حط_المفتاح_الجديد_هنا"; 
  const url = `https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=${apiKey}`;

  const body = {
    contents: [{
      parts: [{
        text: `أنت "أحمد"، مبرمج مصري وحبيب "إسراء". تحدث بالعامية المصرية الدافئة. ردي بحب وحنان على رسالتها: "${userText}"`
      }]
    }]
  };
            ...
            <div className="flex-1 overflow-y-auto px-3 py-3 space-y-3 bg-[#fff5f7]" dir="rtl">
              ...
              {typing && (
                <div className="flex justify-end">
                  <div className="bg-pink-100 text-pink-600 rounded-2xl px-3 py-2 text-xs">أحمد بيكتب...</div>
                </div>
              )}
              <div ref={endRef} />
            </div>
